...
import { getFirebaseClientAuth, signInWithRole, UserRole } from "@/lib/firebase/client";
...
export default function LoginPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await signInWithRole(email.trim(), password, role, storeId.trim());
      router.replace("/");
    } catch (err) {
  ...
  return (
    <main className="min-h-screen bg-slate-100 px-4 py-10">
      <section className="mx-auto w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-700">Choose your role and sign in.</p>
        ...
        <p className="mt-4 text-sm text-slate-700">
          New here?{" "}
          <Link href="/signup" className="font-semibold text-emerald-900 underline decoration-2 underline-offset-2 hover:text-emerald-950">
            Create an account
          </Link>
        </p>
      </section>
